...
static int ntfsMounting = 0;
static int ntfsMountSucc = 0;
...
int getNtfsMountSucc()
{
    return ntfsMountSucc;
}
int ntfsMount(char *srcDev , char *dstDir)
{
	...
	opts.device = srcDev;
    opts.mnt_point = dstDir;
    opts.options = "-o noac";
    //opts.options = "-o dirsync";
	if (ntfs_fuse_init()) {
	...
	if (failed_secure)
	        ntfs_log_info("%s\n",failed_secure);
	if (permissions_mode)
	        ntfs_log_info("test %s, configuration type %d\n",permissions_mode,
			4 + POSIXACLS*6 - KERNELPERMS*3 + CACHEING);
